...
import {babyWeight} from '../../../config/WebServices';
...
export default class App extends Component {
  ...
  sendBabyTemp = async () => {
    ...
    createResource(babyWeight, payload, _.token)
      .then((res) => {
        Alert.alert('Success', 'Successfully Added Baby Weight!');
      })
      .catch((error) => {
  ...
  _renderMyList = ({item}) => (
    <View style={{flex: 1}}>
      <View style={style.renderListContainer}>
        ...
        <Text style={style.renderListHeading1}>{item}</Text>
      </View>
    </View>
  );
  openToggel = () => {
          ...
          <View style={comStyle.TemperatureView}>
            <View style={comStyle.Temperature}>
              <View style={comStyle.weighHeaderContainer}>
                <View style={{flex: 1, paddingLeft: width * 0.005}}>
                  <Text style={comStyle.currentTempText}>Current Weight</Text>
                  <View style={comStyle.boxUperStyle}>
                    <TouchableOpacity
                      style={comStyle.likeInputOxygen}
                      onPress={() => this.openToggel()}>
                      <Text style={comStyle.tempInputCur}>
                        {this.state.currentWei}
                      </Text>
                    </TouchableOpacity>
                    {!this.props.value ? (
                      <Text style={comStyle.kgUper}>kg</Text>
                    ) : (
                      <Text style={comStyle.pondUper}>lb</Text>
                    )}
                    <TouchableOpacity
                      style={comStyle.iconOpenRow}
                    ...
                    <TouchableOpacity
                      style={comStyle.zeroBotton}
                      onPress={() => this.setState({currentWei: 0})}>
                      <Text style={comStyle.zeroText}>Auto Zero</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              </View>
                ...
                <View style={{flex: 1, paddingLeft: width * 0.005}}>
                  ...
                  <View style={comStyle.boxUperStyle}>
                    ...
                    {!this.props.value ? (
                      ...
                      <Text style={comStyle.pondUper}>£</Text>
                    )}
            ...
            <View style={style.insertWeightInner}>
              <View style={style.insertWeightHeader}>
                <View style={style.inserWeightHeaderInner}>
                  ...
                  <Text style={style.inserHeaderHeading}>Baby Weight</Text>
                </View>
              </View>
              <View style={style.inserBodyConatiner}>
                <View style={style.inserBodyInner}>
                  <Text style={style.inserTypeHeading}>Type Weight</Text>
                  ...
                  {!this.props.value ? (
                    <Text style={style.inserConditionUnit}>kg</Text>
                  ) : (
                    <Text style={style.inserConditionUnit}>£</Text>
                  )}
                </View>
              </View>
              <View style={style.inserFooterContainer}>
                <View style={style.inserFooterInner}>
                  <TouchableOpacity
                    style={style.inserSubmitBottonView}
                    onPress={this.submitCondition}>
                    <Text style={style.inserSubmitText}>Submit</Text>
                    <Check
                      name={'checkcircle'}
                      size={width * 0.012}
                      color="white"
          ...
          <View style={{flex: 1}}>
            <View style={style.haiderContainer}>
              <View style={style.headerInner}>
                <Text style={style.headerHeading}>Weight History</Text>
                <View>
                  <TouchableOpacity
                    onPress={() => this.toggleModal()}
                    style={style.submit}>
            ...
            <View style={style.bodyMainContainer}>
              <View style={style.bodyContainer}>
                <View style={style.historyContentHeadingView}>
                  ...
                  <Text style={style.headingTime}>Time</Text>
                  <Text style={style.headingWeight}>Weight</Text>
                </View>
